...
export class VediListaComponent implements OnInit {
  ...
  recoverElement(el: number) {
    this.ms.recoverDeletedElement(this.indexLista, el).subscribe((response: ServerResponse) => {
      if (response.esito === 0) {
        this.elementi.push(this.recoverElementi.find(x => x.id == el));
        this.recoverElementi = this.recoverElementi.filter(e => {
          return e.id != el;
        });
        this.ms.AlertEmitter.emit({ msg: 'Elemento recuperato', color: 'green' });
        if (this.recoverElementi.length === 0)
  ...
  recover() {
    if (this.recovering) {
      this.recovering = false;
      this.recoverElementi = [];
    }
    else {
      this.recovering = true;
      this.ms.getDeletedElements(this.indexLista).subscribe((response: ServerResponse) => {
  ...
  saveAll() {
    if (confirm("Sei sicuro di voler modificare TUTTI gli elementi di questa lista?")) {
  ...
  ngOnInit() {
    ...
    if (this.ms.listePubbliche.find(x => x.id == this.indexLista) && !this.ms.liste.find(x => x.id == this.indexLista))
      this.isPublic = true;
    if (this.ms.liste.find(x => x.id == this.indexLista) || this.isPublic) {
      if (!this.isPublic)
        this.baseElement = this.ms.liste.find(x => x.id == this.indexLista).fields;
      else
        this.baseElement = this.ms.listePubbliche.find(x => x.id == this.indexLista).fields;
